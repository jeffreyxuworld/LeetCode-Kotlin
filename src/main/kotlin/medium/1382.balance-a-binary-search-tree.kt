...
class Solution1382 {
    ...
    fun balanceBST(root: TreeNodeTest1?): TreeNodeTest1? {
        cnt = 0
        var head = root
        dfs(head)
    ...
    fun dfs(root: TreeNodeTest1?) {
        if (root?.left != null) dfs(root?.left)
        arr[cnt++] = root!!.`val`
        if (root?.right != null) dfs(root?.right)
    }
    
    fun sortedArrayToBST(start: Int, end: Int): TreeNodeTest1? {
        if (start > end) return null
        val mid = start + (end - start) / 2
        var root: TreeNodeTest1? = TreeNodeTest1(arr[mid])
        root?.left = sortedArrayToBST(start, mid-1)
        root?.right = sortedArrayToBST(mid+1, end)
        return root
    }

    fun sortedArrayToBST(n: Int): TreeNodeTest1? {
